...
async def custom_scan_endpoint(
    ...
    # 확인용 보존 모드: 이미지 파일만 cleanup 대상 (JSON은 모두 보존)
    files_to_cleanup = [image_path]
    # 이미지 파일만 정리 대상으로 지정합니다.
    ...
    try:
        ...
        all_vulns = (
            filtered_data.get("grype_only", []) +
            filtered_data.get("trivy_only", []) +
            filtered_data.get("mismatch", [])
        )
        ...
        final_payload = {
            ...
            "scan_job": {
                ...
                # 전체 취약점 수입니다. (scan_jobs.total_vulnerabilities)
        ...
        log(f"[+] 스캔 완료. 취약점: {len(filtered_data['vulnerabilities'])} / 시크릿: {len(filtered_data['secrets'])}")
        # 스캔 완료 로그와 취약점 및 시크릿 개수를 출력합니다.
        ...
        # ── 정리 작업 ──────────────────────────────────────────────────────────
        # 임시 파일 삭제 비활성화 — 결과 확인용으로 현재 폴더에 보존합니다.
        log("[*] 생성된 파일 목록:")
...
def build_scan_report(
    ...
    """
    filtered_data["vulnerabilities"] 목록에서
    source == "grype" / "trivy" 를 분리하여
    scan_reports 테이블에 저장할 통계를 계산합니다.
    """

    vulns = filtered_data.get("vulnerabilities", [])
    # 취약점 목록을 가져옵니다.

    # 도구별 { vulnerability_id → severity } 매핑 구성
    ...
    for v in vulns:
        ...
        if v.get("source") == "grype":
            # Grype 도구 출처면
            grype_map.setdefault(vid, sev)
            # 취약점 ID가 없으면 추가합니다.
        elif v.get("source") == "trivy":
            # Trivy 도구 출처면
...
def count_by_severity(filtered_data: dict) -> dict:
    ...
    for v in filtered_data.get("grype_only", []):
        ...
        sev = (v.get("severity") or "UNKNOWN").upper()
        # 심각도를 대문자로 정규화합니다.
        if sev in severity_counts["grype_only"]:
            # 심각도가 알려진 것이면
            severity_counts["grype_only"][sev] += 1
            # 개수를 증가시킵니다.
        else:
            # 알려지지 않은 심각도면
            severity_counts["grype_only"]["UNKNOWN"] += 1
            # UNKNOWN 개수를 증가시킵니다.
    ...
    for v in filtered_data.get("trivy_only", []):
        ...
        sev = (v.get("severity") or "UNKNOWN").upper()
        # 심각도를 대문자로 정규화합니다.
        if sev in severity_counts["trivy_only"]:
            # 심각도가 알려진 것이면
            severity_counts["trivy_only"][sev] += 1
            # 개수를 증가시킵니다.
        else:
            # 알려지지 않은 심각도면
            severity_counts["trivy_only"]["UNKNOWN"] += 1
            # UNKNOWN 개수를 증가시킵니다.
    ...
    for v in filtered_data.get("mismatch", []):
        ...
        sev = (v.get("severity") or "UNKNOWN").upper()
        # 심각도를 대문자로 정규화합니다.
        if sev in severity_counts["mismatch"]:
            # 심각도가 알려진 것이면
            severity_counts["mismatch"][sev] += 1
            # 개수를 증가시킵니다.
        else:
            # 알려지지 않은 심각도면
            severity_counts["mismatch"]["UNKNOWN"] += 1
            # UNKNOWN 개수를 증가시킵니다.
